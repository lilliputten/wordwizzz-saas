...
export function WordsSetsError({ error, reset, className }: TErrorProps) {
  const errText = getErrorText(error);
  React.useEffect(() => {
    const errText = getErrorText(error);
    // eslint-disable-next-line no-console
    console.error('[WordsSetsPage:error]', errText, {
      error,
    });
  ...
  return (
    <ErrorPlaceHolder className={cn(className, '__WordsSetsError')}>
